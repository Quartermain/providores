...
final class VQMod {
	...
	public function modCheck($sourceFile) {
		...
		$cache_enabled = (MijoShop::get('base')->getConfig()->get('enable_vqmod_cache', 1) == 1);
		if ($cache_enabled and file_exists($cacheFile) and (filemtime($cacheFile) >= $this->_lastModifiedTime) and (filemtime($cacheFile) >= $file_last_modified)) {
			return $cacheFile;
		}
