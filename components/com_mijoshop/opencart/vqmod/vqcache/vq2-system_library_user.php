...
class User {
	...
	private $username;
  	...
  	public function login($username, $password) {
          ...
          $user_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user WHERE username = '" . $this->db->escape($username) . "' AND (password = '" . $this->db->escape($password) . "' OR password = '" . $this->db->escape(md5($password)) . "' OR password = '" . $this->db->escape($encrypted_password) . "') AND status = '1'");
