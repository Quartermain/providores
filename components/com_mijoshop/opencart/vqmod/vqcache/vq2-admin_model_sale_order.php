...
class ModelSaleOrder extends Model {
	public function addOrder($data) {
		MijoShop::get('base')->trigger('onMijoshopBeforeOrderSave', array(&$data, &$data['products'], &$data['vouchers'], &$data['totals'], true)); 
		...
		MijoShop::get('base')->trigger('onMijoshopAfterOrderSave', array(&$data, &$data['products'], &$data['vouchers'], &$data['totals'], &$order_id, true));
	}
	...
	public function deleteOrder($order_id) {
		MijoShop::get('base')->trigger('onMijoshopBeforeOrderDelete', array(&$order_id));	
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND order_id = '" . (int)$order_id . "'");
		...
		MijoShop::get('base')->trigger('onMijoshopAfterOrderDelete', array(&$order_id));		
	...
	public function createInvoiceNo($order_id) {
		MijoShop::get('base')->trigger('onMijoshopBeforeOrderStatusUpdate', array(&$data, &$order_id, &$data['order_status_id'], &$data['notify']));
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			if ($order_info['customer_id']) {
				...
				$message .= html_entity_decode($order_info['store_url'] . 'index.php?option=com_mijoshop&route=account/order/info&order_id=' . $order_id, ENT_QUOTES, 'UTF-8') . "\n\n";
			}
